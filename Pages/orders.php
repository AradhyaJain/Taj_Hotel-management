   ...
   if(!isset($_SESSION['name']))
	...
	{       if(empty($_POST['quantity']))
		...
		else
		{
			...
			$q="select price from orders where item='$product'";
			$result=mysqli_query($conn,$q);
...
<?php 
	...
	if (mysqli_num_rows($result) > 0 && mysqli_num_rows($res) > 0) {	 
		  ...
		  echo "<label>Quantity</label><input type='number' name='quantity' value='1'>";
		  echo "<br><br>";
		  $dodo="<input align='center' type='submit' value='Place order'>";
		  echo $dodo;
		  echo "<br><br>";
		 $result=mysqli_query($conn,$q);
