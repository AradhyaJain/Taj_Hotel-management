<?php
   ...
   if(!isset($_SESSION['name']))
	...
	if($_SERVER["REQUEST_METHOD"] == "POST")
	{
			...
			if(mysqli_num_rows($result) > 0)
			{
				$q="DELETE FROM `orders` WHERE `orders`.`item` = '$item'";
				mysqli_query($conn,$q);
				$itemErr="Item removal complete";
			
			}
			else
			{
				$itemErr="No items present";
			}
	}
	}
?>
...
<?php 
	...
	if (mysqli_num_rows($result) > 0) {
		  ...
		  echo "</select>";
		  echo "<br>";
		  $dodo="<input align='center' type='submit' value='Remove item'>";
		  echo $dodo;
	}
	else{
