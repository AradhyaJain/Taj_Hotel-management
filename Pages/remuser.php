<?php
   ...
   if(!isset($_SESSION['name']))
	...
	{       
			$item=$_POST['email'];
			if($item==$_SESSION['name'])
			{
				$itemErr="Cannot remove self account!";
			}
			else
			{
			$q="select * from users where email='$item'";
			$result=mysqli_query($conn,$q);
			if(mysqli_num_rows($result) > 0)
			{
				$q="DELETE FROM `users` WHERE `users`.`email` = '$item'";
				mysqli_query($conn,$q);
				$itemErr="User removal complete";
			}
			else
			{
				$itemErr="No other users present";
			}
			}
	}
	}
...
<?php 
	...
	if (mysqli_num_rows($result) > 0) {
		  ...
		  echo "</select>";
		  echo "<br>";
		  $dodo="<input align='center' type='submit' value='Remove user'>";
		  echo $dodo;
	}
	else{
		echo "No other users present";
	} ?>
</form>
