<?php
   ...
   if(!isset($_SESSION['name']))
	...
	if($_SERVER["REQUEST_METHOD"] == "POST")
	{
			...
			if(mysqli_num_rows($result) > 0)
			{
				$q="DELETE FROM `services` WHERE `services`.`service` = '$item'";
				mysqli_query($conn,$q);
				$itemErr="Service removal complete";
			
			}
			else
			{
				$itemErr="No service present";
			}
	}
	}
?>
...
<?php 
    $q="select service from services";
    $result=mysqli_query($conn,$q);
	if (mysqli_num_rows($result) > 0) {
		  ...
		  echo "</select>";
		  echo "<br>";
		  $dodo="<input align='center' type='submit' value='Remove service'>";
		  echo $dodo;
	}
	else{
		echo "No service present";
	} ?>
</form>
